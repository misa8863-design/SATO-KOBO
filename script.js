const openingOverlay = document.querySelector("[data-opening]");
const openingVideo = openingOverlay?.querySelector(".opening-video");
...
if (openingOverlay && openingVideo) {
  ...
  const closeOpening = () => {
    ...
    openingVideo.pause();
    setTimeout(() => openingOverlay.remove(), 900);
  };
  ...
  const playOpening = openingVideo.play();
  if (playOpening && typeof playOpening.catch === "function") {
    playOpening.catch(() => {
      openingVideo.muted = true;
      openingVideo.play().catch(closeOpening);
    });
  }

  setTimeout(closeOpening, 30000);
} else {
  document.body.classList.remove("intro-active");
}
